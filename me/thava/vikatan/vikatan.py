...
class Vikatan:
    ...
    def __init__(self):
        ...
        self.email_server = _init_mail_server()
        self._login()

    def parse_vikatan(self, name, home_response):
        # print home_response
        for link in BeautifulSoup(home_response, parse_only=SoupStrainer('a')):
            if "href" in getattr(link, 'attrs', {}) and link['href'].find('article.php') != -1 and link['href'].find(
                    'news.vikatan.com') == -1:
                url = link['href']
                print name, url
    ...
    def read_article(self, name, url):
        ...
        self.articles.append([article_id, name, soup.title.string, mail_content])
